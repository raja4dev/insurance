node{
        ...
        stage('build docker image'){
            sh 'sudo docker build -t mrajasekhar13/insure-me:1.0 .'
        }
        ...
        stage('push docker image to docker hub registry')
        {
            ...
            withCredentials([string(credentialsId: 'docker-hub-password', variable: 'dockerHubPassword')]) {
                sh "sudo docker login -u mrajasekhar13 -p ${dockerHubPassword}"
                sh 'sudo docker push mrajasekhar13/insure-me:1.0'
            }
        }
